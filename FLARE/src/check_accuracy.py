...
class error:
    ...
    def test_set(self):
        ...
        results = {'r2': r2_,
                    'mape': mape_,
                    'rmse': rmse_,
                    'range_nrmse_': range_nrmse_,
                    'std_nrmse_': std_nrmse_,
                    'max_rmse_': max_rmse_,
                    'max_range_nrmse': max_range_nrmse_,
                    'nmax_ae': nmax_ae_,
                    'paper_rmse': paper_rmse,
                    'paper_rmse_max': paper_rmse_max}
    ...
    def forward_model_get_results(self, function_name, sampler):
        ...
        # model = self.model[0].fit(self.model[1], self.model[-1])
        
        prediction_output = self.model.predict(test_inputs)
        # prediction_output = model.predict(test_inputs)
